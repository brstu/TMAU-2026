...
11. Сборка проекта должна осуществляться с помощью системы сборки CMake. Запуск
    сборки должен осуществляться через  `.bat` файл и должен автоматически
    демонстрировать прохождение процесса сборки (для уточнения или примера смотри ЛР№2)

### Дополнительное задание (Advanced Level): (Необязательное)

12. **Анализ устойчивости систем:** Для выбранных линейных дискретных моделей
